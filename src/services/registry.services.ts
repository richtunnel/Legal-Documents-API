...
export class DocumentServiceRegistry {
  ...
  constructor(private eventBus: EventBus, private logger: any) {}
  ...
  async registerService(service: ServiceDefinition): Promise<void> {
    try {
      ...
      await redisClient.setEx(serviceKey, this.TTL, JSON.stringify(service));

      // Store health separately for faster queries
      await redisClient.setEx(healthKey, this.TTL, JSON.stringify(service.health));

      // Add to services list
      await redisClient.sAdd(`services_list:${service.name}`, service.id);
  ...
  async discoverServices(serviceName: string): Promise<ServiceDefinition[]> {
    try {
      const serviceIds = await redisClient.sMembers(`services_list:${serviceName}`);
  ...
  async updateHealth(serviceId: string, status: ServiceHealth["status"], checks: HealthCheck[] = []): Promise<void> {
    try {
      const healthKey = `${this.HEALTH_PREFIX}${serviceId}`;
      const existingHealthData = await redisClient.get(healthKey);
      ...
      if (existingHealthData) {
        ...
        health.responseTime = checks.reduce((sum, check) => sum + check.responseTime, 0) / (checks.length || 1); // Example: Average response time
      } else {
        health = {
          status,
          checks,
      ...
      await redisClient.setEx(healthKey, this.TTL, JSON.stringify(health));
  ...
  async deregisterService(serviceId: string): Promise<void> {
    try {
      ...
      const keys = await redisClient.keys(`${this.SERVICE_PREFIX}*:${serviceId}`);

      if (keys.length > 0) {
        const serviceData = await redisClient.get(keys[0]);
        if (serviceData) {
          const service = JSON.parse(serviceData) as ServiceDefinition;

          // Remove from Redis
          await Promise.all([redisClient.del(keys[0]), redisClient.del(`${this.HEALTH_PREFIX}${serviceId}`), redisClient.sRem(`services_list:${service.name}`, serviceId)]);
